...
uses
  ...
  FireDAC.VCLUI.Wait, AdvSmoothButton;
...
type
  TFacceso = class(TForm)
    ...
    oBtn_OK: TAdvSmoothButton;
    oBtn_Cancel: TAdvSmoothButton;
...
begin
  ...
  self.iPassOk := false;
  cVer := '';
  cVer := fUtilesV20.Sto_GetFmtFileVersion(ParamStr(0),'%d.%d.%d');
  self.caption := self.caption + ' Ver. ' + cVer;
...
begin
  ...
  iId_User := oQryUser.FieldByName('u_Id').AsInteger;
...
begin
  try
    ...
    begin
      ...
      oSqlStr.Clear;
      oSqlStr.Lines.Clear;
      oSqlStr.Lines.Add('SELECT emp_id,emp_descripcion FROM mant_empresas ');
      ...
      begin
        ...
        sSqlCmd := 'INSERT INTO `gen_empresas` (`emp_descripcion`, `emp_carpeta_reportes`, `emp_inactivo`) VALUES ("Empresa Pruebas", "' + ExtractFilePath(ParamStr(0)) +
          'Reportes"' + ', 1)';
