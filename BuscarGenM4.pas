...
procedure TfBuscarGenM4.BitBtn1Click(Sender: TObject);
var
  nIndex: Integer;
begin
  oGrid.Visible := false;
...
procedure TfBuscarGenM4.BitBtn9Click(Sender: TObject);
begin
  vFindResult := '';
  self.close;
end;

procedure TfBuscarGenM4.oFilterKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TfBuscarGenM4.oGridDblClick(Sender: TObject);
begin
  vFindResult := self.oGrid.SelectedField.DataSet.FieldByName(cCampoLLave).AsString;
  close;
end;

procedure TfBuscarGenM4.oGridTitleClick(Column: TColumnEh);
{$J+}
const
...
procedure TfBuscarGenM4.oLst_List1CloseUp(Sender: TObject; Accept: Boolean);
begin
  if (self.oLst_List1.KeyValue <> null) then
  begin
    self.cLst_Valu := self.oLst_List1.KeyValue;
    self.Cargar_Lst2();
  end;
end;

procedure TfBuscarGenM4.FormCreate(Sender: TObject);
var
  I: Integer;
begin
...
procedure TfBuscarGenM4.FormShow(Sender: TObject);
var
  nIndex: Integer;
begin
  self.Cargar_Lst1();
...
procedure TfBuscarGenM4.Grid_Show;
var
  nIndex: Integer;
  oColumnX: TColumnEh;
begin
...
procedure TfBuscarGenM4.SetGridColumnWidths(Grid: TDBGridEh);
{
  const
  DEFBORDER = 10;
}
...
procedure TfBuscarGenM4.Cargar_Lst1();
begin
  ...
  self.cLst_Valu := self.oQry_Lst1.FieldByName(self.cLst1_Key_Fld).AsString;
end;

procedure TfBuscarGenM4.Cargar_Lst2();
begin
  ...
  if (trim(self.cLst2_Key_Fld_Lnk) <> '') then
  begin
    self.oQry_Lst2.Filter := self.cLst2_Key_Fld_Lnk + '=' + QuotedStr(self.cLst_Valu);
