...
begin
  bDuplica := false;
  cCod_Rzg := trim(oGrid_Rzg_Lst.DataSource.DataSet.FieldByName('codigo_factor').AsString);
  ...
  begin
    ...
    cCod_Cte := self.cCte_Sel;

  if (bDuplica = false) then
  begin
    cSql_Ln := '';
    cSql_Ln := cSql_Ln + 'INSERT INTO cliente_factores_riesgo ';
    cSql_Ln := cSql_Ln + '  (id_cliente	,codigo_factores_riesgo	,ultima_fecha_actualizacion	,ultimo_usuario_que_actualizo	, factor_check,flag_tmp) VALUES ';
    cSql_Ln := cSql_Ln + '  ("' + cCod_Cte + '","' + cCod_Rzg + '",NOW(),"' + trim(IntToStr(UtilesV20.iUserID)) + '",0,1) ';
...
begin
  if (self.iOption = 1) then
    cwhereCte := 'WHERE (cliente_factores_riesgo.id_cliente = "' + trim(self.cTmp_Sec) + '") '
  else
  begin
    // self.cCte_Sel := trim(self.otCliente_Nat.FieldByName('id_cliente').AsString);
    if (trim(self.cCte_Sel) = '') then
      self.cCte_Sel := 'ABC-123';
    cwhereCte := '';
    cwhereCte := cwhereCte + 'WHERE (cliente_factores_riesgo.id_cliente = "' + trim(self.cCte_Sel) + '") ';
    if ((self.iOption = 1) or (self.iOption = 2)) then
...
begin
  if (self.iTipPer = 1) then
  begin
    bDoble_Nac := self.odoble_nacionalidad.Checked;
  end
  else
    bDoble_Nac := false;
  ...
  if (bDoble_Nac = false) then
  begin
    cSql_Ln := '';
    cSql_Ln := cSql_Ln + 'DELETE FROM cliente_factores_riesgo ';
    cSql_Ln := cSql_Ln + 'WHERE ( (id_cliente="' + trim(self.cTmp_Sec) + '") OR (id_cliente="' + cCod_Cte + '") ) ';
    cSql_Ln := cSql_Ln + 'AND   (codigo_factores_riesgo="FP02")';
    UtilesV20.Execute_SQL_Command(cSql_Ln);
  end
  else
  begin
    cSql_Ln := '';
    cSql_Ln := cSql_Ln + 'INSERT INTO cliente_factores_riesgo ';
    cSql_Ln := cSql_Ln + '	(id_cliente	,codigo_factores_riesgo	,ultima_fecha_actualizacion	,ultimo_usuario_que_actualizo	, factor_check) VALUES ';
    cSql_Ln := cSql_Ln + '	("' + cCod_Cte + '", "FP02", NOW(),"' + trim(IntToStr(UtilesV20.iUserID)) + '",0) ';
    cSql_Ln := cSql_Ln + 'ON DUPLICATE KEY UPDATE ';
    cSql_Ln := cSql_Ln + '	ultimo_usuario_que_actualizo="' + trim(IntToStr(UtilesV20.iUserID)) + '", ';
    cSql_Ln := cSql_Ln + '	ultima_fecha_actualizacion  =NOW() ';
    UtilesV20.Execute_SQL_Command(cSql_Ln);
  ...
  begin
    ...
    cSql_Ln := cSql_Ln + 'DELETE FROM cliente_factores_riesgo ';
    cSql_Ln := cSql_Ln + 'WHERE ( (id_cliente="' + trim(self.cTmp_Sec) + '") OR (id_cliente="' + cCod_Cte + '") ) ';
    cSql_Ln := cSql_Ln + 'AND   (codigo_factores_riesgo="FP01")';
    UtilesV20.Execute_SQL_Command(cSql_Ln);
  end
  else
  begin
    ...
    cSql_Ln := cSql_Ln + 'INSERT INTO cliente_factores_riesgo ';
    cSql_Ln := cSql_Ln + '	(id_cliente	,codigo_factores_riesgo	,ultima_fecha_actualizacion	,ultimo_usuario_que_actualizo	, factor_check) VALUES ';
    cSql_Ln := cSql_Ln + '	("' + cCod_Cte + '", "FP01", NOW(),"' + trim(IntToStr(UtilesV20.iUserID)) + '",0) ';
    cSql_Ln := cSql_Ln + 'ON DUPLICATE KEY UPDATE ';
    cSql_Ln := cSql_Ln + '	ultimo_usuario_que_actualizo="' + trim(IntToStr(UtilesV20.iUserID)) + '", ';
    cSql_Ln := cSql_Ln + '	ultima_fecha_actualizacion  =NOW() ';
    UtilesV20.Execute_SQL_Command(cSql_Ln);
