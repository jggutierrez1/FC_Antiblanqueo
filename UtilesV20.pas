...
type
  TfUtilesV20 = class(Tform)
    ...
    function TimeADD(cTime: string; iHours: integer = 1; iMins: integer = 0; iSecs: integer = 0): string;
    function TimeDiff_to_Hour(pcTimeI: string = '08:00:00'; pcTimeF: string = '08:00:00'): extended;
    ...
    function LastDayOfMonth(const pDateTime: TDateTime): string;
    procedure Date_Range_Of_type(const pDateTime: TDateTime; pType: String; var pDateIni: string; var pDateEnd: string);
...
function fields_info(oConn_Tmp: TFDConnection; var oQryFlds: TFDQuery; field: string = '*'; DBName: string = ''; TableName: string = ''): Boolean; overload;
...
function Exec_Select_SQL(var oQry: TFDQuery; sSql: string; oQryExec: Boolean = True; oQryCreate: Boolean = True; IsProcedure: Boolean = False; ExecNoOpen: Boolean = False)
  : Boolean; overload;
function Exec_Select_SQL(var oQry: TFDQuery; sSql: TStringList; oQryExec: Boolean = False; oQryCreate: Boolean = True; IsProcedure: Boolean = False; ExecNoOpen: Boolean = False)
  : Boolean; overload;
function Exec_Select_SQL(oConn_Tmp: TFDConnection; var oQry: TFDQuery; sSql: string; oQryExec: Boolean = False; oQryCreate: Boolean = True; IsProcedure: Boolean = False;
  ExecNoOpen: Boolean = False): Boolean; overload;
...
begin
  try
    ...
    begin
      ...
      self.oPublicCnn.FormatOptions.FmtDisplayDate := 'yyyy-mm-dd';
      self.oPublicCnn.FormatOptions.FmtDisplayDateTime := 'yyyy-mm-dd hh:mm:ss';
      self.oPublicCnn.FormatOptions.FmtDisplayTime := 'hh:mm:ss';

      self.oZeosConn.Connected := False;
      self.oZeosConn.Properties.Clear;
      self.oZeosConn.Properties.Add('CLIENT-MULTI_ATATEMENTS=1');
      self.oZeosConn.HostName := oSetting_Fac.host;
      self.oZeosConn.Port := oSetting_Fac.puerto;
      self.oZeosConn.database := oSetting_Fac.database;
      self.oZeosConn.Catalog := oSetting_Fac.database;
      self.oZeosConn.User := oSetting_Fac.usuario;
      self.oZeosConn.Password := oSetting_Fac.clave;
      self.oZeosConn.protocol := oSetting_Fac.protocol;

      if self.Check_Host_IsAlive(oSetting_Fac.host, oSetting_Fac.puerto) = True then
      begin
        self.oPublicCnn.Connected := True;
        self.oZeosConn.Connected := True;
      end
      else
      begin
        bConnectionOk := False;
...
begin
  ...
  cSql := 'SELECT  * FROM gen_empresas WHERE emp_activo=1 ORDER BY emp_descripcion ASC ';
  oQry1.SQL.text := cSql;
  oQry1.Open;
...
begin
  Result := Execute_SQL_Command('CREATE DATABASE IF NOT EXISTS `' + trim(cTMP_Database) + '` /*!40100 CHARACTER SET utf8 COLLATE utf8_general_ci */;');
end;
...
begin
  ...
  cSql_Line := 'SELECT DATE_FORMAT(DATE_ADD("2000-01-01 ' + cTime + '",INTERVAL ' + IntToStr(nValue) + ' ' + cInterval + '),"%T") AS xTime ';
...
begin
  ...
  if Exec_Select_SQL(oQry_Sec_CD, 'SELECT suc_Id,no_estacion,no_operacion FROM  conf_sistema WHERE suc_Id= ' + QuotedStr(IntToStr(iId_Empresa)) + ' AND no_estacion= ' +
  ...
  Execute_SQL_Command('UPDATE conf_sistema SET no_operacion = no_operacion + 1 WHERE suc_Id= ' + QuotedStr(IntToStr(iId_Empresa)) + ' AND no_estacion= ' +
    QuotedStr(cId_Estacion) + '; ');
...
begin
  ...
  begin
    case nOption of
      1:
        Execute_SQL_Command('UPDATE conf_sistema SET no_cargoinv     = no_cargoinv + 1 WHERE suc_Id= ' + QuotedStr(IntToStr(iId_Empresa)) + ' AND no_estacion= ' +
          QuotedStr(cId_Estacion) + ' ;');
      2:
        Execute_SQL_Command('UPDATE conf_sistema SET no_descargoinv  = no_descargoinv + 1  WHERE suc_Id= ' + QuotedStr(IntToStr(iId_Empresa)) + ' AND no_estacion= ' +
          QuotedStr(cId_Estacion) + ' ;');
      3:
        Execute_SQL_Command('UPDATE conf_sistema SET no_ordenprod    = no_ordenprod + 1    WHERE suc_Id= ' + QuotedStr(IntToStr(iId_Empresa)) + ' AND no_estacion= ' +
          QuotedStr(cId_Estacion) + ' ;');
      4:
        Execute_SQL_Command('UPDATE conf_sistema SET no_cargoprod    = no_cargoprod + 1    WHERE suc_Id= ' + QuotedStr(IntToStr(iId_Empresa)) + ' AND no_estacion= ' +
          QuotedStr(cId_Estacion) + ' ;');
      5:
        Execute_SQL_Command('UPDATE conf_sistema SET no_descargoprod = no_descargoprod + 1 WHERE suc_Id= ' + QuotedStr(IntToStr(iId_Empresa)) + ' AND no_estacion= ' +
          QuotedStr(cId_Estacion) + ' ;');
      6:
        Execute_SQL_Command('UPDATE conf_sistema SET no_tomainv      = no_tomainv + 1      WHERE suc_Id= ' + QuotedStr(IntToStr(iId_Empresa)) + ' AND no_estacion= ' +
          QuotedStr(cId_Estacion) + ' ;');
    end;
    if Exec_Select_SQL(oQry_Sec_CD, 'SELECT * FROM  conf_sistema WHERE suc_Id=' + QuotedStr(IntToStr(iId_Empresa)) + ' AND no_estacion= ' + QuotedStr(cId_Estacion) + ' ;', True,
      True) = True then
    begin
      case nOption of
        1:
...
procedure TfUtilesV20.PrintLineOnDotMatrix(cPrinterName: string; cDocName: string; cString: string; bDobleCarr: Boolean; bTest: Boolean = False);
// Uses WinSpool;
var
  handle: THandle;
  n: DWORD;
...
function TfUtilesV20.TimeADD(cTime: string; iHours: integer = 1; iMins: integer = 0; iSecs: integer = 0): string;
var
  cTimeAdd: string;
  cSql_Ln: string;
  cResult: string;
  cTmp1, cTmp2, cTmp3, cTmp4: string;
begin
  if (iHours > 12) then
  begin
    showmessage('La hora permitida es de 0 a 12 horas.');
    Result := cTime;
  end;
  if (iMins > 60) then
  begin
    showmessage('Los minutos permitidos son de 0 a 60 minutos.');
    Result := cTime;
  end;
  cTmp1 := cTime;
  cTmp2 := StringReplace(cTmp1, 'AM', ' ', [rfReplaceAll, rfIgnoreCase]);
  cTmp3 := StringReplace(cTmp2, 'PM', ' ', [rfReplaceAll, rfIgnoreCase]);
  cTimeAdd := trim(IntToStr(iHours)) + ':' + trim(IntToStr(iMins)) + ':' + trim(IntToStr(iSecs));
  cSql_Ln := 'SELECT ADDTIME("' + trim(cTmp3) + '","' + trim(cTimeAdd) + '")';
  cResult := UtilesV20.Execute_SQL_Result(cSql_Ln);
  Result := cResult;
end;

function TfUtilesV20.TimeDiff_to_Hour(pcTimeI: string = '08:00:00'; pcTimeF: string = '08:00:00'): extended;
var
  cxTime: string;
  iSecs: integer;
  fHours: extended;
  ...
  cResult: string;
begin
  cSql_Ln := 'SELECT TIMEDIFF("' + trim(pcTimeF) + '","' + trim(pcTimeI) + '")';
  cxTime := UtilesV20.Execute_SQL_Result(cSql_Ln);
  iSecs := fUtilesV20.TimeToSecs(cxTime);
  fHours := (iSecs / 3600);
  Result := fHours;
end;

function TfUtilesV20.tiempo_Trans_Result(pStartTime: DWORD; pEndTime: DWORD; iFlag: integer = 0): string;
var
  ElapsedTime: DWORD;
  Estimated_hours: DWORD;
  Estimated_Minutes: DWORD;
...
function TfUtilesV20.Replace_Day_in_Datetime(pDateTime: TDateTime; pDay: integer): TDateTime;
begin
  Result := RecodeDate(pDateTime, YearOf(pDateTime), MonthOf(pDateTime), pDay);
end;

function TfUtilesV20.Replace_Month_in_Datetime(pDateTime: TDateTime; pMonth: integer): TDateTime;
begin
  Result := RecodeDate(pDateTime, YearOf(pDateTime), pMonth, DayOf(pDateTime));
end;

function TfUtilesV20.Replace_Year_in_Datetime(pDateTime: TDateTime; pYear: integer): TDateTime;
begin
  Result := RecodeDate(pDateTime, pYear, MonthOf(pDateTime), DayOf(pDateTime));
end;

function TfUtilesV20.Replace_Hour_in_Datetime(pDateTime: TDateTime; pHour: integer): TDateTime;
begin
  Result := RecodeTime(pDateTime, pHour, MinuteOf(pDateTime), SecondOf(pDateTime), 0);
end;

function TfUtilesV20.Replace_Minute_in_Datetime(pDateTime: TDateTime; pMinute: integer): TDateTime;
begin
  Result := RecodeTime(pDateTime, HourOf(pDateTime), pMinute, SecondOf(pDateTime), 0);
end;

function TfUtilesV20.Add_Days_in_Datetime(pDateTime: TDateTime; pDay: integer): TDateTime;
begin
  Result := fUtilesV20.DateTimeAdd(pDateTime, dtpDay, pDay);
end;

function TfUtilesV20.Add_Months_in_Datetime(pDateTime: TDateTime; pMonth: integer): TDateTime;
begin
  Result := fUtilesV20.DateTimeAdd(pDateTime, dtpMonth, pMonth);
end;

function TfUtilesV20.Add_Years_in_Datetime(pDateTime: TDateTime; pYear: integer): TDateTime;
begin
  Result := fUtilesV20.DateTimeAdd(pDateTime, dtpYear, pYear);
end;

function TfUtilesV20.Add_Hours_in_Datetime(pDateTime: TDateTime; pHour: integer): TDateTime;
begin
  Result := fUtilesV20.DateTimeAdd(pDateTime, dtpHour, pHour);
end;

function TfUtilesV20.Add_Minutes_in_Datetime(pDateTime: TDateTime; pMinute: integer): TDateTime;
begin
  Result := fUtilesV20.DateTimeAdd(pDateTime, dtpMinute, pMinute);
end;

function TfUtilesV20.DateForSQL(const pDate: TDate): string;
var
  y, m, d: Word;
begin
...
function TfUtilesV20.DateTimeForSQL(const pDateTime: TDateTime): string;
begin
  Result := FormatDateTime('yyyy-mm-dd hh:nn:ss', pDateTime);
end;

function TfUtilesV20.LastDayOfMonth(const pDateTime: TDateTime): string;
var
  cFecha: string;
  cSql_Ln: STRING;
begin
  cFecha := FormatDateTime('yyyy-mm-dd', pDateTime);

  cSql_Ln := 'SELECT LAST_DAY("' + trim(cFecha) + '")';
  Result := UtilesV20.Execute_SQL_Result(cSql_Ln);
end;

procedure TfUtilesV20.Date_Range_Of_type(const pDateTime: TDateTime; pType: String; var pDateIni: string; var pDateEnd: string);
var
  cFecha: string;
  cSql_Ln: string;
  iAct_Ano, iAct_Mes: integer;
  cAct_Ano, cAct_Mes: string;
  cfecha_FinMs: string;
begin
  iAct_Ano := YearOf(pDateTime);
  iAct_Mes := MonthOf(pDateTime);

  cAct_Ano := trim(IntToStr(iAct_Ano));
  cAct_Mes := trim(IntToStr(iAct_Mes));

  cfecha_FinMs := fUtilesV20.LastDayOfMonth(pDateTime);
  if (pType = '01') then
  begin
    cSql_Ln := 'SELECT STR_TO_DATE(CONCAT(' + cAct_Ano + ', "-", LPAD(' + cAct_Mes + ', 2, "00"), "-01 06:01:01"),"%Y-%m-%d %H:%i:%s")';
    pDateIni := UtilesV20.Execute_SQL_Result(cSql_Ln);

    cSql_Ln := 'SELECT STR_TO_DATE(CONCAT(' + cAct_Ano + ', "-", LPAD(' + cAct_Mes + ', 2, "00"), "-15 06:01:01"), "%Y-%m-%d %H:%i:%s")';
    pDateEnd := UtilesV20.Execute_SQL_Result(cSql_Ln);
  end
  else if (pType = '02') then
  begin
    cSql_Ln := 'SELECT STR_TO_DATE(CONCAT(' + cAct_Ano + ', "-", LPAD(' + cAct_Mes + ', 2, "00"), "-16 06:01:01"),"%Y-%m-%d %H:%i:%s")';
    pDateIni := UtilesV20.Execute_SQL_Result(cSql_Ln);

    cSql_Ln := 'SELECT CONCAT(' + cfecha_FinMs + ', " 23:59:59")';
    pDateEnd := UtilesV20.Execute_SQL_Result(cSql_Ln);
  end
  else if (pType = '03') then
  begin
    cSql_Ln := 'SELECT STR_TO_DATE(CONCAT(' + cAct_Ano + ', "-", LPAD(' + cAct_Mes + ', 2, "00"), "-01 06:01:01"),"%Y-%m-%d %H:%i:%s")';
    pDateIni := UtilesV20.Execute_SQL_Result(cSql_Ln);
...
procedure TfUtilesV20.TxtFile_Write(FileName: string; cFull_Text: string);
var
...
function TfUtilesV20.Sto_GetFmtFileVersion(const FileName: String = ''; const Fmt: String = '%d.%d.%d.%d'): String;
var
  sFilename: String;
  iBufferSize: DWORD;
  iDummy: DWORD;
