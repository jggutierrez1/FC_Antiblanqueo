...
type
  TfBuscarGenM3 = class(TForm)
    ...
    procedure Cargar_Lst();
  private
  public
    cLst_Key_Fld: string;
    cLst_Lst_Fld: string;
    cLst_Sql_Cmd: Widestring;
    cLst_Val_Def: string;
    cLst_Where_Fld: string;
  end;
...
procedure TfBuscarGenM3.BitBtn1Click(Sender: TObject);
var
  nIndex: Integer;
begin
  oGrid.Visible := false;
  ...
  oSqlTmp.Lines.Add(' AND ' + self.cLst_Where_Fld + '= " ' + trim(self.oLst_List.KeyValue) + ' " ');
...
procedure TfBuscarGenM3.BitBtn9Click(Sender: TObject);
begin
  vFindResult := '';
  self.close;
end;

procedure TfBuscarGenM3.oFilterKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TfBuscarGenM3.oGridDblClick(Sender: TObject);
begin
  vFindResult := self.oGrid.SelectedField.DataSet.FieldByName(cCampoLLave).AsString;
  close;
end;

procedure TfBuscarGenM3.oGridTitleClick(Column: TColumnEh);
{$J+}
const
...
procedure TfBuscarGenM3.FormCreate(Sender: TObject);
var
  I: Integer;
begin
  self.oQry_Lst.Connection := self.oConection;
  self.oLst_List.ListSource := nil;
  self.oLst_List.ListField := '';
  self.oLst_List.KeyField := '';
  self.oLst_List.Clear;
...
procedure TfBuscarGenM3.FormShow(Sender: TObject);
var
  nIndex: Integer;
begin
  self.Cargar_Lst();
...
procedure TfBuscarGenM3.Grid_Show;
var
  nIndex: Integer;
  oColumnX: TColumnEh;
begin
...
procedure TfBuscarGenM3.SetGridColumnWidths(Grid: TDBGridEh);
{
  const
  DEFBORDER = 10;
}
...
procedure TfBuscarGenM3.Cargar_Lst();
begin
  self.oQry_Lst.close();
  self.oQry_Lst.Connection := fUtilesV20.oPublicCnn;
  self.oQry_Lst.SQL.Clear;
  self.oQry_Lst.SQL.Text := self.cLst_Sql_Cmd;
  self.oQry_Lst.Open();
  self.oDs_Lst.DataSet := self.oQry_Lst;
  self.oDs_Lst.enabled := true;

  self.oLst_List.ListSource := self.oDs_Lst;
  self.oLst_List.ListField := self.cLst_Lst_Fld;
  self.oLst_List.KeyField := self.cLst_Key_Fld;
  self.oLst_List.Refresh;
  self.oLst_List.KeyValue := self.cLst_Val_Def;
end;
